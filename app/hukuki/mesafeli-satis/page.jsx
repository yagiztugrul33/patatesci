export const metadata = {
  title: "Mesafeli Satış Sözleşmesi | patatesçi — Tarımsal Ürün Ticaret Platformu",
  description: "patatesçi platformu mesafeli satış sözleşmesi.",
};

export default function MesafeliSatis() {
  return (
    <main className="section">
      <div className="container legal">
        <p className="eyebrow">Hukuki bilgilendirme</p>
        <h1>Mesafeli Satış Sözleşmesi</h1>
        <p>
          Platform üzerinden gerçekleştirilen işlemlere ilişkin mesafeli satış
          sözleşmesi bu sayfada yer alacaktır.
        </p>
        <p>Bu metin hukuk danışmanı onayı sonrası yayınlanacaktır.</p>
      </div>
    </main>
  );
}
