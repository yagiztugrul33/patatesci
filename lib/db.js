...
// Soyut ödeme katmanı (demo→gerçek PSP köprüsü): sipariş makinesi yalnız bu
// dört fonksiyonu tanır; sağlayıcı env ile değişir (docs/ODEME-KARARI.md).
import { tahsilat as pspTahsilat, guvence as pspGuvence, split as pspSplit, iade as pspIade, PLATFORM_KOMISYON_ORANI_TEMSILI } from "./odeme.mjs";
  ...
  while (changed) {
    ...
    if (bid.fiyat >= ask.fiyat && bid.userId !== ask.userId) {
      ...
      const order = {
        ...
        // Ödeme izi (DEMO): soyut katman lib/odeme.mjs üzerinden yürür.
        // Gerçek PSP bağlanınca aynı hareket türleri gerçek işlem no'larıyla dolar.
        odeme: { saglayici: "demo", hareketler: [] },
...
// ---- Satıcı onboarding (demo, gerçek alanlarla) ----
// Akış: başvuru (künye/ÇKS/sertifika) → "dogrulama_bekliyor" → yönetim onay/red.
// Onay mevcut akışları KAPILAMAZ (API sözleşmesi korunur); "Onaylı Üye"
// rozeti ve ileride gerçek HKS eşleşmesi için zemin hazırlar.
export function onboardingBasvur({ userId, kunyeNo, cksNo, bolge, sertifikalar }) {
  const db = read();
  const u = db.users.find((x) => x.id === userId);
  if (!u) return { ok: false, reason: "Kullanıcı bulunamadı." };
  const kunye = String(kunyeNo || "").trim();
  const cks = String(cksNo || "").trim();
  if (kunye.length < 4 && cks.length < 4) {
    return { ok: false, reason: "Künye numarası veya ÇKS numarasından en az biri gereklidir." };
  }
  db.onboarding = db.onboarding || [];
  const mevcut = db.onboarding.find((b) => b.userId === userId && b.durum === "dogrulama_bekliyor");
  if (mevcut) return { ok: false, reason: "Bekleyen bir doğrulama başvurunuz zaten var." };
  const basvuru = {
    userId,
    ad: u.ad,
    email: u.email,
    ...
    sertifikalar: Array.isArray(sertifikalar) ? sertifikalar.map((s) => String(s).slice(0, 60)).slice(0, 5) : [],
    durum: "dogrulama_bekliyor",
    ts: Date.now(),
    karar: null,
  };
  db.onboarding.push(basvuru);
  write(db);
  return { ok: true, basvuru };
}

export function onboardingDurum(userId) {
  const db = read();
  const b = (db.onboarding || []).filter((x) => x.userId === userId).sort((a, c) => c.ts - a.ts)[0] || null;
  return { basvuru: b, onayliUye: !!db.users.find((u) => u.id === userId)?.onayliUye };
}

export function onboardingListe() {
  const db = read();
  return (db.onboarding || []).slice().sort((a, b) => b.ts - a.ts);
}

export function onboardingKarar({ userId, karar, gerekce }) {
  if (karar !== "onay" && karar !== "red") return { ok: false, reason: "Karar 'onay' veya 'red' olmalı." };
  const db = read();
  const b = (db.onboarding || []).find((x) => x.userId === Number(userId) && x.durum === "dogrulama_bekliyor");
  if (!b) return { ok: false, reason: "Bekleyen başvuru bulunamadı." };
  b.durum = karar === "onay" ? "onaylandi" : "reddedildi";
  b.karar = { karar, gerekce: String(gerekce || "").slice(0, 300), ts: Date.now() };
  const u = db.users.find((x) => x.id === b.userId);
  if (u) u.onayliUye = karar === "onay";
  write(db);
  return { ok: true, basvuru: b };
}

// Yönetim paneli sipariş izleme (salt okuma, kişisel alanlar kırpılmış değil —
// panel zaten anahtar korumalı; e-posta operasyon için gereklidir).
export function tumSiparisler() {
  const db = read();
  return (db.orders || []).map((o) => ({
    id: o.id, urun: o.urun, ton: o.ton, fiyat: o.fiyat, tutar: o.tutar,
    durum: o.durum, satici: o.satici, alici: o.alici, cezaSayisi: (o.cezalar || []).length,
    odemeHareket: (o.odeme?.hareketler || []).map((h) => h.tur).join(">"),
  }));
}
...
export function updateOrder(id, aksiyon, ek = {}) {
  ...
  if (aksiyon === "yeniden-onay") {
    ...
    // Ödeme izi (DEMO): güvencedeki bedelin iadesi — alıcı iptalinde ceza mahsup edilir,
    // satıcı iptalinde alıcıya TAM iade (ceza satıcı teminatından tazminat olur)
    o.odeme = o.odeme || { saglayici: "demo", hareketler: [] };
    const iadeTutar = taraf === "alici" ? Math.max(0, o.tutar - c.ceza - (c.nakliyeCezasi || 0)) : o.tutar;
    if (iadeTutar > 0) {
      const h = pspIade({ siparisId: o.id, tutar: iadeTutar, sebep: `İptal (${asama}) — ${taraf} iptali${taraf === "alici" ? ", ceza mahsuplu" : ""}` });
      if (h.ok) { o.odeme.hareketler.push(h); o.gecmis.push(`Ödeme hareketi: İADE ${TL(h.tutar)} alıcıya (${h.islemId}, demo)`); }
    }
    ...
    // Ödeme izi (DEMO — soyut katman): durum geçişine karşılık gelen para hareketi
    o.odeme = o.odeme || { saglayici: "demo", hareketler: [] };
    if (o.durum === "goruntulu_onay_bekliyor") {
      const h = pspTahsilat({ siparisId: o.id, tutar: o.tutar });
      if (h.ok) { o.odeme.hareketler.push(h); o.gecmis.push(`Ödeme hareketi: TAHSİLAT ${TL(h.tutar)} (${h.islemId}, demo)`); }
    } else if (o.durum === "odeme_guvencede") {
      const h = pspGuvence({ siparisId: o.id, tutar: o.tutar });
      if (h.ok) { o.odeme.hareketler.push(h); o.gecmis.push(`Ödeme hareketi: GÜVENCE BLOKESİ ${TL(h.tutar)} (${h.islemId}, demo)`); }
    } else if (o.durum === "tamamlandi") {
      const h = pspSplit({ siparisId: o.id, tutar: o.tutar, komisyonOran: PLATFORM_KOMISYON_ORANI_TEMSILI });
      if (h.ok) {
        o.odeme.hareketler.push(h);
        o.gecmis.push(`Ödeme hareketi: SPLIT — üretici ${TL(h.saticiPay)} + platform komisyonu ${TL(h.komisyon)} (temsilî %${(PLATFORM_KOMISYON_ORANI_TEMSILI * 100).toLocaleString("tr-TR")}, ${h.islemId}, demo)`);
      }
    }
