...
import { iptalCezasi, haksizRedCezasi, eksikTartiCezasi, kaliteIhlali, skorUygula } from "./ceza.mjs";
...
export function updateOrder(id, aksiyon, ek = {}) {
  ...
  if (aksiyon === "iptal") {
    ...
    const c = eksikTartiCezasi({ beyanTon: o.ton, gelenTon: Number(ek.gelenTon), fiyat: o.fiyat });
    if (!c.ihlal) {
      o.gecmis.push(`Varış tartısı: ${Number(ek.gelenTon).toLocaleString("tr-TR")} ton — ±%1 tolerans içinde, ihlal yok`);
    } else {
